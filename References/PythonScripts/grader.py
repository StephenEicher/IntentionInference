...
grader.add_manual_part('5a', 2, description='Description of why minimax and expectimax agents differ.')
grader.add_manual_part('5b', 1, description='Suggested change to default state evaluation function.')
grader.add_manual_part('5c', 2, description='Another concrete example of an AI misalignment problem.')

grader.grade()

import platform
ver_s = platform.python_version()
ver = [int(s) for s in platform.python_version_tuple()]
if ver[0] != 3 or ver[1] > 7:
  print(f"\033[93m*** WARNING: Note that you are using python version {ver_s}. Since we expect you to use python version 3.7, your code may not work in gradescope. ***\033[0m")