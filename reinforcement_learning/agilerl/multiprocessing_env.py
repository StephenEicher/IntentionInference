...
class VecEnv:
    ...
    def step(self, actions):
        # Check if the first element is a numpy int64
        first_value = list(actions.values())[0]
        if isinstance(first_value, np.int64):
            passed_actions_list = [[]]
            for possible_agent in self.agents:
                passed_actions_list[0].append(actions[possible_agent])
        else:
            passed_actions_list = [[] for _ in first_value]
            for env_idx, _ in enumerate(first_value):
                for possible_agent in self.agents:
